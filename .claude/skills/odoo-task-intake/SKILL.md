...
**18.0**), reutilizando el código fuente y los módulos nativos.
...
- Si **no** lo dio → **pregunta** cuál usar, ofreciendo las disponibles:
  ...
  ls /workspace/*.conf
  ```
  (p. ej. `odoo.conf`, `odoo-uysa.conf`, `odoo-atika.conf`, `odoo-proriego.conf`, `odoo-andes.conf`,
  `odoo-icon-supply.conf`, `odoo-evi-dji.conf`, `odoo-sisegusa.conf`). Confirma también el
  **repo/módulo** objetivo en `/workspace/custom-addons/<cliente>/`.
...
git -C <repo> branch -a                            # ramas; base = main/staging/18.0 (excluye mirrors OCA)
```
...
- **(a) Crear rama nueva desde producción**: `git -C <repo> fetch` y crear `feat/<tarea>-<slug>` (o
  ...
  `staging-*` en `pe-uysa`/`pe-andesnutrition`, `18.0` en `nextconnector`, la rama de feature en
  `atika`, o `main` según el repo), o
...
`hotfix/<tarea>-<slug>`, `release/<tarea>-<slug>`, `features/<...>`.

## Paso 4 — Desarrollo + validación LOCAL (obligatorio antes de proponer commit)
a. **Implementa** reutilizando módulos nativos de Odoo y el código fuente en `/workspace/odoo.18.0`
   y `/workspace/enterprise.18.0`. **Bump del manifest** (`version`) del módulo tocado.
b. **Crea tests unitarios**:
   - Backend: `TransactionCase`/`HttpCase` con los tags del módulo.
   - Frontend/OWL (si aplica): **Hoot** (bundle `web.assets_unit_tests`), incl. estilos con
     `toHaveStyle`. Ver skill `odoo-web-validation`.
...
- **Idioma**: español en repos de cliente (pe-uysa, atika, pe-andesnutrition, nextconnector). Respeta
  el estilo nativo del repo (algunos usan Conventional Commits `feat(scope):`).
