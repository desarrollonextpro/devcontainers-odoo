...
**16.0**), reutilizando el código fuente y los módulos nativos.
...
- Si **no** lo dio → **pregunta** cuál usar, ofreciendo las disponibles:
  ...
  ls /workspace/*.conf
  ```
  (p. ej. `odoo.conf`, `odoo-sego.conf`, `odoo-ec-marriott.conf`). Confirma también el **repo/módulo**
  objetivo en `/workspace/custom-addons/` (aquí los repos de cliente son `sego` y `ec-b2b-marriott`).
...
git -C <repo> branch -a                            # ramas; base = main/staging/16.0 (excluye mirrors OCA)
```
...
- **(a) Crear rama nueva desde producción**: `git -C <repo> fetch` y crear `feat/<tarea>-<slug>` (o
  ...
  `staging` en `sego`, la rama de feature en `ec-b2b-marriott`, o `16.0`/`main` según el repo), o
...
a. **Implementa** reutilizando módulos nativos de Odoo y el código fuente en `/workspace/odoo.16.0`
   y `/workspace/enterprise.16.0`. **Bump del manifest** (`version`) del módulo tocado.
b. **Crea tests unitarios**:
   - Backend: `TransactionCase`/`HttpCase` con los tags del módulo.
   - Frontend/OWL (si aplica): **QUnit** (Odoo 16 no tiene Hoot), incl. validación de estilos con
     `getBoundingClientRect`/`getComputedStyle`. Ver skill `odoo-web-validation`.
...
- **Idioma**: español en repos de cliente (sego, ec-b2b-marriott). Respeta el estilo nativo del repo
  (algunos usan Conventional Commits `feat(scope):`).
