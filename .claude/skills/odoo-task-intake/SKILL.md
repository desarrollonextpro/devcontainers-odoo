...
**19.0**), reutilizando el código fuente y los módulos nativos.
...
- Si **no** lo dio → **pregunta** cuál usar, ofreciendo las disponibles:
  ...
  ls /workspace/*.conf
  ```
  (p. ej. `odoo.conf`, `nextpro-operaciones.conf`, `nextpro-helpdesk-ai.conf`, `nextpro-v19-test.conf`,
  `sisegusa-billing-flow*.conf`, `molinorte-visit-flow.conf`…). Confirma también el **repo/módulo**
  objetivo en `/workspace/custom-addons/`.
...
git -C <repo> branch -a                            # ramas; base = main o 19.0 (excluye mirrors OCA)
```
...
- **(a) Crear rama nueva desde producción**: `git -C <repo> fetch` y crear `feat/<tarea>-<slug>` (o
  `release/<tarea>-<slug>`, `fix/<tarea>-<slug>`) **desde la rama base/producción** (`main` en repos
  maduros como pe-uysa/nextpro-operaciones; `19.0` en repos mono-rama), o
...
a. **Implementa** reutilizando módulos nativos de Odoo y el código fuente en `/workspace/odoo.19.0`
   y `/workspace/enterprise.19.0`. **Bump del manifest** (`version`) del módulo tocado.
b. **Crea tests unitarios**:
   - Backend: `TransactionCase`/`HttpCase` con los tags del módulo.
   - Frontend/OWL (si aplica): **Hoot** (v17-19) o **QUnit** (≤v16), incl. estilos con `toHaveStyle`.
     Ver skill `odoo-web-validation` y memoria [[hoot-tests-odoo19]].
...
- **Idioma**: español en repos de cliente (pe-uysa, nextpro-operaciones, nextconnector); inglés en
  repos de producto (next_flow_ai, next_flow_whatsapp, nextpro_meta_portal). Respeta el estilo nativo
  del repo (algunos usan Conventional Commits `feat(scope):`).
...
- **No hagas** PR, merge ni `git push` hasta que el usuario lo **autorice explícitamente y tras haber
  probado**. El push a `19.0`/producción es especialmente sensible (dispara sync a prod vía
  cloudpepper-agent). Un "ok al commit" **no** es "ok al push". Ver [[pedir-autorizacion-push]].
