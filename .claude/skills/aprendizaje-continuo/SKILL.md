...
2. **Clasifica** cada aprendizaje y decide su destino con esta tabla de routing:
   ...
   | Conocimiento **específico de una versión** de Odoo (API, QUnit vs Hoot, cambios entre versiones…) | Archivo por versión: `/workspace/.claude/knowledge/odoo-*-17.md` (p. ej. `odoo-owl-components-17.md`) |
...
> 1. (por versión) La sub-versión 17.0 de este contenedor usa Hoot/`<list>` (o QUnit/`<tree>` si es un
>    snapshot temprano) → propongo añadir 1 línea a `/workspace/.claude/knowledge/odoo-version-knowledge-17.md`.
