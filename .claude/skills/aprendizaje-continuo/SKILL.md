...
description: >-
  ...
  conocimiento por versión, skill de un componente (p. ej. nextconnector), o memoria. Sirve para no
  repetir errores recurrentes (estilos, vistas, desarrollo, colas, buenas prácticas) entre sesiones.
---
...
Inspirado en el "Nivel 4" de la skill `registrar-soporte`.
...
2. **Clasifica** cada aprendizaje y decide su destino con esta tabla de routing:
   ...
   | Conocimiento **específico de una versión** de Odoo (API, Hoot vs QUnit, cambios v17→v19…) | Archivo por versión: `/workspace/.claude/knowledge/odoo-*-<v>.md` (p. ej. `odoo-owl-components-19.md`) |
   | Conocimiento de un **componente/producto** (nextconnector, next_flow_ai, next_flow_whatsapp, next_billing_flow, next-visit-flow, nextpro_meta_portal…) — y **dependiente de su versión** | Skill del componente: `/workspace/.claude/skills/<componente>/SKILL.md` (crea la skill si no existe) |
...
> 1. (genérico) En Odoo 19 los `<tree>` se llaman `<list>` → propongo añadir 1 línea a
>    `/workspace/.claude/skills/odoo-version-knowledge-19.md`.
> 2. (componente) La cola de `nextconnector_base` reintenta con backoff X → propongo añadir a
>    `/workspace/.claude/skills/nextconnector/SKILL.md` sección "cola".
>
