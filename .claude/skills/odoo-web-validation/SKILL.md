...
description: >-
  ...
  tests nativos de navegador (Hoot en Odoo 17-19, QUnit en ≤16) y tours. Úsala cuando haya que
  "revisar cómo se ve", "validar el flujo/estilos", "tomar una captura", comprobar que nada se
  desborde ni se sobreponga, o correr tests de frontend/OWL.
---
...
**Versión de este contenedor: Odoo 19.0.** Las secciones marcadas *(portabilidad)* indican qué
cambia en otras versiones (útil al replicar la skill a v15-18).
...
1. **Asegura que Odoo está sirviendo.** Comprueba salud:
   ...
   ```
   Si no responde, levántalo con la `.conf` de la instancia objetivo (puerto según la conf; p. ej.
   `nextpro-helpdesk-ai.conf` usa 8072). Ejecutar en background:
   ```bash
   /workspace/.venv/bin/python /workspace/odoo.19.0/odoo-bin -c <conf> &
   ```
2. **Navega y loguéate** con las herramientas del MCP (`mcp__playwright__browser_navigate`, `_type`,
   `_click`). URL: `http://localhost:8069/odoo` (o `/web` en versiones previas *(portabilidad)*).
   Credenciales según la instancia (para la BD de test v19, ver memoria [[nextpro-v19-test-db]]). Login estándar Odoo: campo `login`, campo `password`, botón submit.
...
## C. Tests nativos de frontend (Hoot / QUnit)

**Odoo 19 (y 17-18): Hoot.** Ver la memoria [[hoot-tests-odoo19]] para el detalle completo y gotchas.
Resumen:
- Registrar el bundle en el `__manifest__.py`: `"web.assets_unit_tests": ["<módulo>/static/tests/**/*"]`.
- Correr sobre BD aislada (sin demo), con `addons_path` que evite módulos con QWeb roto:
  ```bash
  /workspace/.venv/bin/python /workspace/odoo.19.0/odoo-bin -c <conf> -d <db_test> \
...
**Odoo ≤16: QUnit** *(portabilidad)*. NO existe Hoot. El bundle es `web.qunit_suite_tests`
(o `web.tests`); helpers en `web.test_utils`; se corre con `--test-tags` de QUnit y se valida estilo
con aserciones sobre `el.getBoundingClientRect()`/`window.getComputedStyle`. Al replicar a v16 o
menor, reescribir esta sección para QUnit.

**Tours** (funciona en todas las versiones con chromium): `HttpCase.start_tour` / `browser_js`
...
## Gotchas
- Usa **BD de test aislada** para Hoot (`-i <módulo> --without-demo=all`), no la BD de trabajo.
- Evita en el `addons_path` de test los módulos con QWeb roto que revientan el bundle (histórico:
  `whatsapp_connector` en `nextpro-operaciones`). Ver [[hoot-tests-odoo19]].
- Si el MCP `playwright` no está disponible (Node 18 / sin rebuild), cae a: chromium headless directo
