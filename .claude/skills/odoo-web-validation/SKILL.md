...
description: >-
  ...
  tests nativos de navegador (Hoot en Odoo 18) y tours. Úsala cuando haya que "revisar cómo se
...
**Versión de este contenedor: Odoo 18.0.** Odoo 18 usa **Hoot** para los tests de frontend, la URL
de cliente web es **`/odoo`**, y las vistas de lista se declaran con **`<list>`**. Las secciones
marcadas *(portabilidad)* indican qué cambia en otras versiones (v16 usa QUnit + `/web` + `<tree>`).
...
1. **Asegura que Odoo está sirviendo.** Comprueba salud:
   ...
   ```bash
   /workspace/.venv/bin/python /workspace/odoo.18.0/odoo-bin -c <conf> &
   ```
   > Odoo escucha internamente en `8069`; el puerto publicado hacia el host es `18069` (docker-compose),
...
2. **Navega y loguéate** con las herramientas del MCP (`mcp__playwright__browser_navigate`, `_type`,
   `_click`). URL: `http://localhost:8069/odoo` (en v16 es `/web` *(portabilidad)*).
...
## C. Tests nativos de frontend (Hoot en Odoo 18)

**Odoo 18 usa Hoot** (bundle `web.assets_unit_tests`). Resumen:
...
- Correr sobre BD aislada (sin demo), con `addons_path` que evite módulos con QWeb roto:
  ```bash
  /workspace/.venv/bin/python /workspace/odoo.18.0/odoo-bin -c <conf> -d <db_test> \
...
- Patrones Hoot: `mountWithCleanup(...)`, `defineMailModels()`, `contains(...)`, `onRpc(...)`.
  Los imports vienen de `@odoo/hoot`, `@odoo/hoot-dom`, `@odoo/hoot-mock`.
- **Estilos/layout en Hoot**: `expect(selector).toHaveStyle({...})` / `not.toHaveStyle(...)`.

*(portabilidad — v16):* usa **QUnit** (bundle `web.qunit_suite_tests`, helpers `web.test_utils`, sin
Hoot). Al portar esta skill a un contenedor v16, reescribe esta sección para QUnit.
